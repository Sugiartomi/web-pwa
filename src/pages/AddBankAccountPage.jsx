...
function AddBankAccountPage() {
	...
	const handleSubmit = (event) => {
		...
		// if (state.status === "add") dispatch(add_bank_account({ bankAccountName, bankCode, accountNumber }));
		// else navigate(`/withdraw/${state.pair}`, { state: state.state });
	};
	...
	return (
		<>
			<form action="" className="mt-2" onSubmit={handleSubmit}>
				<div className={ darkMode ? "container text-roboto text-white" : "container text-roboto"} style={{ minHeight : "100vh"}}>
					...
					<div className="d-flex align-items-center">
						<p className="fw-bold" onClick={() => navigate(`/withdraw/${state.pair}`, { state: state.state })}>
							<ChevronLeft />
						</p>
					...
					<div className="mb-3">
						...
						<div className="d-flex align-items-center">
							<input type="text" className={darkMode ? "form-control shadow-none border-0 bg-dark-mode2 text-white" : "form-control shadow-none border-0"} value={fullName} style={{ backgroundColor: "#DDE4EA", height: "54px" }} />
						</div>
					</div>
					...
					<div className="mb-3">
						...
						<select className={ darkMode ? "form-select font-16 bg-dark-mode2 text-white border-0" : "form-select font-16"} aria-label="Default select example" onChange={(e) => setBankCode(e.target.value)} style={{ height: "54px", border: "1px solid #DDE4EA" }}>
					...
					<div className="mb-3">
						...
						<div className="d-flex align-items-center">
							<input type="number" className={ darkMode ? "form-control shadow-none bg-dark-mode2 text-white border-0" : "form-control shadow-none"} placeholder="Enter the bank account number you choose" style={{ border: "1px solid #DDE4EA", height: "54px" }} onChange={(e) => setAccountNumber(e.target.value)} />
						</div>
					</div>
					...
					{state.status === "edit" ? (
						<div className="row gx-2">
							<div className="d-grid col-6">
								<button type="submit" className="btn font-red" style={{ border: "1px solid #F4465E" }}>
									Delete
								</button>
							</div>
							<div className="d-grid col-6">
								<button type="submit" className="btn text-white" style={{ backgroundColor: "#1B6AEA" }}>
									Done
								</button>
							</div>
						</div>
				...
				{state.status === "add" ? (
					<>
						<div className="fixed-button pb-2 px-2 col-12 width-breakpoint">
							<div className="d-grid col-12">
								<button type="submit" className="btn text-white" style={{ backgroundColor: "#1B6AEA", height: "46px" }} data-bs-toggle="modal" data-bs-target="#modal-account-verification">
									Next
								</button>
							</div>
						</div>
			...
			<div className="modal modal-sm custom fade profile-info text-roboto rounded-0" id="modal-account-verification" tabIndex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
				<div className="modal-dialog modal-dialog-centered justify-content-center">
					<div className="modal-content rounded border-0" id="modal-addBankAccount" style={{ width: "350px" }}>
						<div className="modal-body">
							...
							<p className="text-center font-yellow fw-bolder mb-0" style={{ fontSize: "50px" }}>
								<FaRegClock />
							</p>
							<p className="font-14 text-center font-grey2">We will verify your bank account account within 1x24 hours.</p>
							{/* button ok */}
							<div className="d-grid col-12">
								<button type="button" className="btn text-white" style={{ backgroundColor: "#1B6AEA" }} data-bs-dismiss="modal" onClick={() => navigate(`/withdraw/${state.pair}`)}>
									OK
								</button>
							</div>
						</div>
